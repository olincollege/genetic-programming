...
from parse_tree import ParseTree, TerminalGenerationRules
...
class IrisGP:
  ...
  def fitness_function(self, tree, X, y, target_class=-1):
    ...
    for i in range(len(X)):
      ...
      if target_class >= 0:
        predicted_class = target_class if predicted_value > 0 else \
...
def run_benchmarks():
  ...
  # Run benchmarks with varying population size
  print("\nBenchmarking with varying population size...")
  for pop_size in population_sizes:
    gp = IrisGP(population_size=pop_size, generations=20, **fixed_params)
    start_time = time.time()
  ...
  # Run benchmarks with varying generations
  print("\nBenchmarking with varying generation count...")
  for gen_count in generation_counts:
    gp = IrisGP(population_size=100, generations=gen_count, **fixed_params)
    start_time = time.time()
    classifiers, _ = gp.solve_multi_class(X_train, y_train)
    end_time = time.time()
    total_time = end_time - start_time
  ...
  plt.tight_layout()
  plt.savefig('python_benchmarks.png')
  plt.close()
...
if __name__ == "__main__":
  ...
  print("\nBenchmark results saved to 'python_gp_benchmarks.csv'")
  print("Benchmark plots saved to 'python_benchmarks.png'")
